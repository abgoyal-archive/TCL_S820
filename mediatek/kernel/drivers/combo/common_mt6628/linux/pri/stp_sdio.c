    ...
    while (!osal_thread_should_stop(&pInfo->tx_rx_thread))
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    STPSDIO_DBG_FUNC("enable COM IRQ okay\n");
                }
                #if 0
                else
                {
        ...
        {
            STPSDIO_DBG_FUNC("sleeping check fail. stp_is_ready(%d) irq_pending(%d) tx_packet_num(%d) rx_pkt_len(%d)\n",
                mtk_wcn_stp_is_ready(),
                pInfo->irq_pending,
    ...
    )
{
    ...
    STPSDIO_HINT_FUNC("disable BGF IRQ\n");
    /*Disable Common interrupt output in CHLPCR */
    ...
    else
    {
        ...
        else
        {
            if (!(chlcpr_value & C_FW_INT_EN_SET))
            {
                STPSDIO_DBG_FUNC("disable COM IRQ okay\n");
