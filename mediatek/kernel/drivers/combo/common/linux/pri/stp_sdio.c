...
#define STPSDIO_LOG_LOUD    4
#define STPSDIO_LOG_DBG     3
...
static INT32 stp_sdio_do_own_clr (void)
{
#define CLR_OWN_RETRY 10
    int ret = -1;
    ...
    STPSDIO_INFO_FUNC("Do FW-Own back!(Wakeup)\n");
...
//need to wait for the ownback completion
    ...
    do {
        ...
        if ((chlcpr_value & C_FW_COM_DRV_OWN) == C_FW_COM_DRV_OWN) {
            ...
            STPSDIO_INFO_FUNC("firmware ownback is polled!(%d)\n", CLR_OWN_RETRY - retry);
            break;
        }
        else {
            STPSDIO_INFO_FUNC("firmware ownback is polled!(%d)\n", CLR_OWN_RETRY - retry);
    ...
    if (time_out != 0) {
        STPSDIO_INFO_FUNC("set firmware ownback is successful!\n");
        ret = 0;
    }
    else {
        STPSDIO_ERR_FUNC("set firmware ownback is fail\n");
        ret = -1;
    }
...
#define SAFE_CNT_TO_SET_OWN 5
    ...
    while (1) {
        ...
        if ((gp_info->pkt_buf.rd_idx == gp_info->pkt_buf.wr_idx)
            ...
            STPSDIO_ERR_FUNC("complete count(%d), tx_packet_num(%d)!\n",
                comp_count, p_info->firmware_info.tx_packet_num);
        }
        else 
        {
            spin_unlock_irqrestore(&gp_info->pkt_buf.rd_idx_lock, gp_info->pkt_buf.rd_irq_flag);
            STPSDIO_INFO_FUNC("set own (sleep) is temporarily not permitted! rd=%d, wr=%d, wait\n", 
...
#endif
        if (retry < SAFE_CNT_TO_SET_OWN) {
            STPSDIO_INFO_FUNC("set own (sleep) is not permitted, bus is busy.\n");
            break;
        }

        if (tx_fifo_clear_cnt == SAFE_CNT_TO_SET_OWN) {
            STPSDIO_INFO_FUNC("set own (sleep) is permitted!\n");
            break;
        }
    }
    ...
    do {
        ...
        if ((chlcpr_value & C_FW_COM_DRV_OWN) == 0) {
            //4 <2> handle ownership back interrupt
            STPSDIO_INFO_FUNC("set firmware own! (retry = %d) is succeful\n", SET_OWN_RETRY - retry);
            break;
        }
        else {
    ...
    )
{
    ...
    case OWN_CLR:
        ret = stp_sdio_do_own_clr();
        break;

    default:
        STPSDIO_WARN_FUNC("op(%d) is no supported\n", op);
        ret = -1;
        break;
    }
...
#if STP_SDIO_NEW_TXRING
    STPSDIO_DBG_FUNC("len(%u), wr_cnt(%u), rd_cnt(%u)\n", 
    pInfo->rx_pkt_len, 
...
#else
    STPSDIO_DBG_FUNC("len(%u), rd_idx(%u), wr_idx(%u)\n", 
    pInfo->rx_pkt_len, 
