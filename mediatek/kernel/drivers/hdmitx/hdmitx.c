...
static HDMI_STATUS hdmi_drv_init(void)
{
    ...
    backup.address = temp_mva;
    backup.x = 0;
    backup.y = 0;
    backup.enable = TRUE;
    backup.pitchInBytes = lcm_width * 3;
    backup.format = LCD_FB_FORMAT_RGB888;
    backup.mode = 3;
    LCD_CHECK_RET(LCD_WriteBackUp(LCD_FB_0, backup));
