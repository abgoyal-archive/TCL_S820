...
static long AKECS_SetMode_SngMeasure(void)
{
	...
	/* Set measure mode */
	buffer[0] = AK8963_REG_CNTL1;
	buffer[1] = AK8963_MODE_SNG_MEASURE;  //14 bit mode
...
static ssize_t show_shipment_test(struct device_driver *ddri, char *buf)
{
	...
	int res = 0;
        ...
        mt_set_gpio_mode(AKM8963_RST, GPIO_MODE_00);
        mt_set_gpio_dir(AKM8963_RST, GPIO_DIR_OUT);
        mt_set_gpio_out(AKM8963_RST, GPIO_OUT_ZERO);
        udelay(10);
        mt_set_gpio_out(AKM8963_RST, GPIO_OUT_ONE);
        udelay(10);
