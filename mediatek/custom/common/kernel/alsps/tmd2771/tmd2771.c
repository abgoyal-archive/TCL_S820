...
static int cal_data[21]={0}; //cal data
/*----------------------------------------------------------------------------*/
...
{
	//printk("john_tmd2771_eint_func start\n");
	...
	//printk("john_tmd2771_eint_func_end\n");
}
...
int tmd2771_setup_eint(struct i2c_client *client)
{
	...
	mt65xx_eint_unmask(CUST_EINT_ALS_NUM);  
	//printk("john_set_eint_end\n");
    return 0;
}

/*----------------------------------------------------------------------------*/

#ifdef DO_CALIBARTION//Enabled by jrd.john for boot-up calibration
...
static int tmd2771_get_ps_value(struct tmd2771_priv *obj, u16 ps)
{
	...
	static int ps_temp[20];
	static int ps_ave=0,ps_sum=0;
        int i,j;
...
//	ps=ps+20;
	...
	printk("john tmd27771_get_ps_value's ps value = %d\n",ps);
	for(i=0;i<20;i++)
	{
	...
	databuf[0] = 0x92;	
	//databuf[1] = 0x12;
	res = i2c_master_send(obj->client, databuf, 0x1);
	if(res <= 0)
		{
			goto EXIT_ERR;
		}
	res = i2c_master_recv(obj->client, reg_value, 0x1);
	if(res <= 0)
		{
			goto EXIT_ERR;
		}
	printk("john_READID:0x%x, %d, %s\n", reg_value[0], __LINE__, __FUNCTION__);

EXIT_ERR:
	APS_ERR("john |read device ID error : %d\n", res);
	return res;
#endif
#endif


	//	printk("john_ps_value=%d,obj->als=%d\n",ps,obj->als);
	//if(ps_cali.valid == 1)
	#if 0
	if(obj->als >= 20)
	
		{
			//APS_LOG("tmd277771_get_ps_value val_temp  = %d",val_temp);
			//if((ps >ps_cali.close)&&(temp_ps[0] >ps_cali.close))
		if((ps  > (atomic_read(&obj->ps_thd_val_high))+200))			
			//if((ps >ps_cali.close))
			{
				val = 0;  /*close*/
				val_temp = 0;
			...
			else if((ps  < (atomic_read(&obj->ps_thd_val_low))+180))
			{
				val = 1;  /*far away*/
				val_temp = 1;
				intr_flag_value = 0;
			...
			//APS_LOG("tmd27771_get_ps_value val  = %d",val);
	}
	else
	#endif
	{
#ifdef JOHN_DEBUG
		printk("john_tmd2771_get_ps_value else branch\n");
		printk("john_ps_value=%d,obj->als=%d\n",ps,obj->als);
#endif
	...
	{
#ifdef JOHN_DEBUG
	    printk("john_debug tmd2771_get_ps_value_ Return value val== %d\n",val);
		APS_DBG("PS:  %05d => %05d\n", ps, val);
#endif
		return val;
	}	
...
//	u8 reg_value[1];
	...
	{
#if  1 //john add use for get-data stable start
			int i;
			int ps_tmp[3];
			int ps_sum=0;
			int ps_ave=0;
		...
		//		printk("john_ps_tmp[%d]=%d\n ",i,ps_tmp[i]);
			...
			ps_ave=ps_sum/3;
#ifdef JOHN_DEBUG
			printk("john_tmd2771_ps_operate read ps valuses ps_sum=%d,ps_ave=%d\n",ps_sum,ps_ave);
#endif 
			obj->ps=ps_ave;
       ...
       unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		//	printk("john_tmd2771_unlocked_ioctl ALSPS_GET_PS_DATA satrt!\n ");
...
static ssize_t tmd2771_show_als(struct device_driver *ddri, char *buf)
{
	...
	if((res = tmd2771_read_als_ch0(g_tmd2771_ptr->client, &g_tmd2771_ptr->als)))
	{
...
static ssize_t tmd2771_show_ps(struct device_driver *ddri, char *buf)
{
	...
	if((res = tmd2771_read_ps(g_tmd2771_ptr->client, &g_tmd2771_ptr->ps)))
	{
		return snprintf(buf, PAGE_SIZE, "ERROR: %d\n", res);
	}
	else
...
static ssize_t tmd2771_show_config(struct device_driver *ddri, char *buf)
{
	...
	res = snprintf(buf, PAGE_SIZE, "(%d %d %d %d %d %d)\n", 
		atomic_read(&g_tmd2771_ptr->i2c_retry), atomic_read(&g_tmd2771_ptr->als_debounce), 
		atomic_read(&g_tmd2771_ptr->ps_mask), atomic_read(&g_tmd2771_ptr->ps_debounce),
	        atomic_read(&g_tmd2771_ptr->ps_thd_val_high),atomic_read(&g_tmd2771_ptr->ps_thd_val_low));     
	return res;    
}
...
static ssize_t tmd2771_store_config(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	{ 
		atomic_set(&g_tmd2771_ptr->i2c_retry, retry);
		atomic_set(&g_tmd2771_ptr->als_debounce, als_deb);
		atomic_set(&g_tmd2771_ptr->ps_mask, mask);
	//	atomic_set(&g_tmd2771_ptr->ps_thd_val, thres);        
		atomic_set(&g_tmd2771_ptr->ps_debounce, ps_deb);
		atomic_set(&g_tmd2771_ptr->ps_thd_val_high, thrh);
		atomic_set(&g_tmd2771_ptr->ps_thd_val_low, thrl);
	}
	else
	{
...
static ssize_t tmd2771_show_status(struct device_driver *ddri, char *buf)
{
	...
	if(g_tmd2771_ptr->hw)
	{
		len += snprintf(buf+len, PAGE_SIZE-len, "CUST:i2c_num= %d\nppcount=%x\ncmm=%x\nhigh=%d\nlow=%d\n", 
			g_tmd2771_ptr->hw->i2c_num, TMD2771_CMM_PPCOUNT_VALUE,  TMD2771_CMM_CONTROL_VALUE,
			g_tmd2771_ptr->hw->ps_threshold_high,g_tmd2771_ptr->hw->ps_threshold_low);
	}
	else
	{
	...
	len += snprintf(buf+len, PAGE_SIZE-len, "REGS: %02X %02X %02lX %02lX\n", 
				atomic_read(&g_tmd2771_ptr->als_cmd_val), atomic_read(&g_tmd2771_ptr->ps_cmd_val), 
				g_tmd2771_ptr->enable, g_tmd2771_ptr->pending_intr);
	

	len += snprintf(buf+len, PAGE_SIZE-len, "MISC: %d %d\n", atomic_read(&g_tmd2771_ptr->als_suspend), atomic_read(&g_tmd2771_ptr->ps_suspend));
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		...
		//		printk("john_ps_tmp[%d]=%d\n ",i,ps_tmp[i]);
			...
			obj->ps=ps_ave;
#endif  //john add use for get-dat stable end.
				//tmd2771_read_ps(obj->client, &obj->ps); //john note:if not use ave data ,please open this line.
				...
				sensor_data->values[0] = tmd2771_get_ps_value(obj, obj->ps);
#ifdef JOHN_DEBUG
				APS_ERR("john_tmd2771_ps_operate ps_data=%d!\n",obj->ps);
#endif
				sensor_data->value_divide = 1;
				sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;			
			}
...
#ifdef JOHN_DEBUG
		printk(KERN_ERR "john tmd2771_ps_calibrate  ps-val = %d XXXXX\n", prox_data[i]);
#endif 
	}
...
#ifdef JOHN_DEBUG
	printk("john tmd2771_ps_calibrate  ps_thd_val_low = %d\n",  atomic_read(&obj->ps_thd_val_low));
	printk("john tmd2771_ps_calibrate  ps_thd_val_high = %d \n", atomic_read(&obj->ps_thd_val_high));
#endif 
