...
static UINT32 OV8850Open(void)
{
	...
	OV8850DB("truly OV8850Open enter :\n ");
...
{
    int  retry = 5; //Modified by zhfan to detect ID for 5 times to ensure detect OK.
    int  check_truly_id = 0;
    int  truly_id = 0;
	
	 OV8850DB("truly OV8850GetSensorID enter :\n ");
    ...
    if(1 == af_driveric_id){
		printk("xxxxxxxx allen not truly first xxxxxxxxxx");
			*sensorID = 0xFFFFFFFF; 
        	return ERROR_SENSOR_CONNECT_FAIL;
		}
	
	printk("xxxxxxxx allen is truly first xxxxxxxxxx");
    ...
    do {
        *sensorID = (OV8850_read_cmos_sensor(0x300A)<<8)|OV8850_read_cmos_sensor(0x300B);
        ...
        if (*sensorID == OV8850_SENSOR_ID)
        	{
        		OV8850DB(" truly Sensor ID = 0x%04x\n", *sensorID);
            OV8850DB(" retry = %d\n", retry);
            	break; 
        	}
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_REGISTER_DEFAULT:
            if (*pFeatureParaLen>=sizeof(NVRAM_SENSOR_DATA_STRUCT))
            {
                ...
                pSensorDefaultData->SensorId=OV8850_SENSOR_ID;
...
{OV8850DB("truly OV8850_MIPI_RAW_SensorInit enter :\n ");
