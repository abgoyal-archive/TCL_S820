...
kal_int32 fgauge_compensate_battery_voltage_recursion_init(kal_int32 ori_voltage, kal_int32 recursion_time)
{
    kal_int32 ret_compensate_value = 0;
    kal_int32 temp_voltage_1 = ori_voltage;
    kal_int32 temp_voltage_2 = temp_voltage_1;
    ...
    {
        gFG_resistance_bat = fgauge_read_r_bat_by_v(temp_voltage_2); // Ohm
        ret_compensate_value = (gFG_current_init * (gFG_resistance_bat + R_FG_VALUE)) / 1000;
        ret_compensate_value = (ret_compensate_value+(10/2)) / 10; // 20101103
    ...
    gFG_resistance_bat = fgauge_read_r_bat_by_v(temp_voltage_2); // Ohm
    ret_compensate_value = (gFG_current_init * (gFG_resistance_bat + R_FG_VALUE + FG_METER_RESISTANCE)) / 1000;    
    ret_compensate_value = (ret_compensate_value+(10/2)) / 10; // 20101103
...
{
    int i=0;
    kal_int32 fg_boot_current;
    kal_int32 UbVbat_tmp;
...
//1. Get Raw Data  
    ...
    {
        if(UbVbat & 0x80000000)  //USB connect from bootloader
         {         
            UbVbat_tmp = UbVbat & 0x7FFFFFFF;
            gFG_voltage = (UbVbat_tmp & 0x7FFF) - 25;          
            fg_boot_current = UbVbat_tmp >> 15;
            if(fg_boot_current == 0)  
             {
                gFG_voltage = UbVbat_tmp + 25;
             } 
           else
             {
                fg_boot_current = (fg_boot_current + 430)*CAR_TUNE_VALUE/100; //test for compensate
                if(chip_diff_trim_value_en == 0){
                fg_boot_current = use_chip_trim_value(fg_boot_current);
                  }
                gFG_current_init = fg_boot_current;
                gFG_voltage = gFG_voltage + fgauge_compensate_battery_voltage_recursion_init(gFG_voltage,5) ; //mV
             }
        }
       else
        {
           UbVbat_tmp = UbVbat;
           gFG_voltage = (UbVbat_tmp & 0x7FFF) - 30;          
           fg_boot_current = UbVbat_tmp >> 15;
           if(fg_boot_current == 0)  
            {
                gFG_voltage = UbVbat_tmp + 30;
            } 
           else
            {
               fg_boot_current = fg_boot_current*CAR_TUNE_VALUE/100;
               if(chip_diff_trim_value_en == 0){
               fg_boot_current = use_chip_trim_value(fg_boot_current);
                 }
               gFG_current_init = fg_boot_current;
...
{    
    ...
    //}
    //jrd add for fuel gauge sometimes work abnormal
    if(gFG_current < 10)
    { 
