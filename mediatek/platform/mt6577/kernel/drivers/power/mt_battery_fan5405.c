...
kal_bool *bat_status_for_tp = NULL;
...
#define TOPOFF_CHG_VOLTAGE      4200    /*another condition to set bat_full flag CV time > 45minutes*/
...
void check_fan5405_fault_status(uint8_t fault_bit, uint8_t work_mode){
        ...
        case BOOST_NORMAL_STATUS:
			...
			fan_struct->ops->set_con2_val(fan_struct, 0x2a, 0x01, 0x01);	/*FIXME: another way to enter boost mode*/
                /*
...
int gpio_number   = GPIO202;		/*Diablo disable pin ??? FIXME*/
...
if (BMT_status.bat_charging_state == CHR_ERROR ) {
    ...
    else
    {
        ...
        if(fan_struct)
                ...
                fan_struct->ops->set_con2_val(fan_struct, 0x2a, 0x01, 0x00);       /*set Vout = 4.20V, otg pin active high, disable otg pin*/
                }
        #endif
...
*/
    ...
    else if(fan_struct && (fan_struct->work_mode == FAN5405_BOOST_MODE))        /*mhl function now*/
    {
        ...
        {
                fan_struct->ops->set_otg_pin(fan_struct, 0x01);                 /*set otg pin output high*/
                fan_struct->ops->set_con2_val(fan_struct, 0x2a, 0x01, 0x01);    /*set to enter boost mode*/
        }
        ...
        {            
            ...
            return;
                ...
                if( (((fan5405_status & 0x30) == 0x20) && (fan_struct->config_count == 0) && (fan_struct->chg_fault_status == CHARGE_NORMAL_STATUS)) ||
					...
					printk("xxxxxxxxxxxxxxxxxxxxxxxxxxxxBMT_status.TOPOFF_charging_time == %d\n",BMT_status.TOPOFF_charging_time);
                                        }
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
    ...
    do
    {
        ...
        if ((upmu_is_chr_det() == KAL_TRUE))
        {
  //          check_battery_exist();	//jrd delete for no battery charging FIXME
        }
...
static int mt6329_battery_probe(struct platform_device *dev)    
{
  ...
  //move to pmic bank1 probe
    ...
    BMT_status.bat_id_valid = 1;
#endif
    bat_status_for_tp = &(BMT_status.charger_exist);
