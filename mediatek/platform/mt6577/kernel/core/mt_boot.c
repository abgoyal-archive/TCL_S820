...
struct chip_desp {
    ...
    bool load;              //!<: is chip description load or not
}; 
...
{
    if (desp->load) return 0;	//skip if chip description is load
    ...
    if (0x8A00 == desp->reg.hw_subcode)
    {
	    ...
	    {
	        printk("[%s] fail to load chip verion: invalid sw_ver(%x)!!", MOD, desp->reg.sw_ver);
            return EINVAL;
	    }
    }
    ...
    {
     	printk("[%s] fail to load chip verion: invalid hw_sub(%x)!!", MOD, desp->reg.hw_subcode);
        return EINVAL;
    }
    desp->load = true;
    return 0;
...
{
    bool flag = (DRV_Reg32(0xF1019100) & 0x01000000) ? (true) : (false);
    unsigned int code;

    load_chip_desp(&g_chip_desp);	
    code = get_chip_code();
    if (0x6575 == code)
	    return (flag == true) ? (0x6515) : (0x6575);
    else if (0x6577 == code)
	    return (flag == true) ? (0x6517) : (0x6577);
    return 0;
}
