...
static void wpa_supplicant_cleanup(struct wpa_supplicant *wpa_s)
{
	...
	wpa_s->l2 = NULL;
	if (wpa_s->l2_br) {
		l2_packet_deinit_2(wpa_s->l2_br);
		wpa_s->l2_br = NULL;
	}
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
    ...
    wpa_printf(MSG_DEBUG, "drv_flags 0x%x",wpa_s->drv_flags);
    wpa_printf(MSG_DEBUG, "freq %u from shared",wpa_drv_shared_freq(wpa_s));
    wpa_printf(MSG_DEBUG, "params.freq %u",params.freq);
...
/* Make sure send DISCONNECT to driver every time */
#if 0
...
int wpa_supplicant_update_mac_addr(struct wpa_supplicant *wpa_s)
{
	if (wpa_s->driver->send_eapol) {
		...
		l2_packet_deinit_2(wpa_s->l2);
		wpa_s->l2 = l2_packet_init_2(wpa_s->ifname,
	...
	if (wpa_s->l2 && l2_packet_get_own_addr_2(wpa_s->l2, wpa_s->own_addr)) {
...
int wpa_supplicant_driver_init(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->bridge_ifname[0]) {
		...
		wpa_s->l2_br = l2_packet_init_2(wpa_s->bridge_ifname,
					      wpa_s->own_addr,
					      ETH_P_EAPOL,
...
#ifdef CONFIG_WAPI_SUPPORT
	wpa_s->l2_wai = l2_packet_init_2(wpa_s->ifname,
					   wpa_drv_get_mac_addr(wpa_s),
					   ETH_P_WAI,
					   wpa_supplicant_rx_wai, wpa_s, 0);

	if (wpa_s->l2_wai && l2_packet_get_own_addr_2(wpa_s->l2_wai, wpa_s->own_addr)) {
