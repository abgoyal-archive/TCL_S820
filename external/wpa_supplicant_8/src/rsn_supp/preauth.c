				  ...
				  size_t len)
{
	...
	res = l2_packet_send_2(sm->l2_preauth, sm->preauth_bssid,
		     ...
		     struct eap_peer_config *eap_conf)
{
	...
	sm->l2_preauth = l2_packet_init_2(sm->ifname, sm->own_addr,
					ETH_P_RSN_PREAUTH,
	...
	if (sm->bridge_ifname) {
		sm->l2_preauth_br = l2_packet_init_2(sm->bridge_ifname,
						   sm->own_addr,
						   ETH_P_RSN_PREAUTH,
						   rsn_preauth_receive, sm, 0);
...
void rsn_preauth_deinit(struct wpa_sm *sm)
{
	...
	l2_packet_deinit_2(sm->l2_preauth);
	sm->l2_preauth = NULL;
	if (sm->l2_preauth_br) {
		l2_packet_deinit_2(sm->l2_preauth_br);
