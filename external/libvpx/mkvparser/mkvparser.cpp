...
bool mkvparser::CheckUIntInvalid(long long data, long len) {
	switch(len){
		case 1:
			return data ^ 0x7fLL;
		case 2:
			return data ^ 0x3fffLL;
		case 3:
			return data ^ 0x1fffffLL;
		case 4:
			return data ^ 0x0fffffffLL;
		case 5:
			return data ^ 0x07ffffffffLL;
		case 6:
			return data ^ 0x03ffffffffffLL;
		case 7:
			return data ^ 0x01ffffffffffffLL;
		case 8:
			return data ^ 0x00ffffffffffffffLL;
		default:
			return false;
	}
}
...
#ifndef ANDROID_DEFAULT_CODE
		if (!CheckUIntInvalid(size, len)) {
			size = total - pos;
			ALOGW("ID=0x%llx, size reserved, set size according to file size->%lld", id, size);
		}
#endif
...
void Track::Info::Clear()
{
    ...
    codecNameAsUTF8 = NULL;
#ifndef ANDROID_DEFAULT_CODE
	ALOGD("Clear: contentAddTotalSize=%d, contentAddData=(%p)", contentAddTotalSize, contentAddData);
	if (contentAddTotalSize != 0) {
		free(contentAddData);
		contentAddTotalSize = 0;
		contentAddData = NULL;
	}	
...
#ifndef ANDROID_DEFAULT_CODE
void Track::Info::ParseContentEncodings(IMkvReader* pReader, long long pos, long long size)
{
	Vector<ContentEncoding::Compression> compressions;
	long long stop = pos+size;
	while (pos < stop) {
		long len;
		const long long id = ReadUInt(pReader, pos, len);
		...
		const long long _size = ReadUInt(pReader, pos, len);
		pos += len;
		...
		if (id == 0x2240) {
			...
			ce.scope = 1;
			ce.type = 0;
			ce.compression.algo = 0;
			ce.compression.settings = NULL;
			ce.compression.settingsSize = 0;
			long long _stop = pos + _size;
			while (pos < _stop) {
				if (Match(pReader, pos, 0x1031, ce.order))
					ALOGD("encoding order=%lld", ce.order);
				else if(Match(pReader, pos, 0x1032, ce.scope))
					ALOGD("encoding scope=%lld", ce.scope);
				else if(Match(pReader, pos, 0x1033, ce.type))
					ALOGD("encoding type=%lld", ce.type);
				else {
					
					long _len;
					
					const long long _id = ReadUInt(pReader, pos, _len);
					assert(_id >= 0);  //TODO: handle error case
					assert((pos + _len) <= _stop);
					
					pos += _len;  //consume id
					
					const long long __size = ReadUInt(pReader, pos, _len);
					assert(__size >= 0);	//TODO: handle error case
					assert((pos + _len) <= _stop);
					
					pos += _len;  //consume length of size
					if (_id == 0x1034) {//ContentCompression
						long long __stop = pos + __size;
						while (pos < __stop) {
							if (Match(pReader, pos, 0x0254, ce.compression.algo))
								ALOGD("compression algo=%lld", ce.compression.algo);
							else if (Match(pReader, 
										   pos, 
										   0x0255, 
										   ce.compression.settings, 
										   ce.compression.settingsSize))
										   ;
							else {
								long __len;
								
								const long long __id = ReadUInt(pReader, pos, __len);
								assert(_id >= 0);  //TODO: handle error case
								assert((pos + _len) <= _stop);
								
								pos += __len;  //consume id
								
								const long long ___size = ReadUInt(pReader, pos, __len);
								assert(___size >= 0);	//TODO: handle error case
								
								pos += __len;  //consume length of size
								pos += ___size; //consume payload
							}
						}
					}
					else
			...
			if ((ce.scope != 1) ||//all frame
				(ce.type != 0) ||//compression
				(ce.compression.algo != 3) ||//header striping
				(ce.compression.settings == NULL)) {//only support this type
				ALOGW("Unsupport content encoding element, scope(%lld), type(%lld), algo(%d), setting size(%d)", 
						ce.scope, ce.type, ce.compression.algo, ce.compression.settingsSize);
			} else {
				ALOGD("content encoding element, encoding scope(%lld), encoding type(%lld), \
						compression algo(%lld), setting size(%d), setting(0x%x)", 
						ce.scope, ce.type, ce.compression.algo, ce.compression.settingsSize, *ce.compression.settings);
				compressions.insertAt(ce.order);
				
				ContentEncoding::Compression* c = &compressions.editItemAt(ce.order);
				c->settings = ce.compression.settings;
				c->settingsSize = ce.compression.settingsSize;
				contentAddTotalSize += ce.compression.settingsSize;
			}
				
		}
		else
			pos += _size; //consume payload
	}

	if (contentAddTotalSize != 0)
	{
		contentAddData = (unsigned char*)malloc(contentAddTotalSize);
		ALOGD("contentAddData=0x%p, contentAddTotalSize=%d", contentAddData, contentAddTotalSize);
		
		size_t offset = 0;
		while (offset < contentAddTotalSize) {
			ContentEncoding::Compression c = compressions.top();
			memcpy(contentAddData + offset, c.settings, c.settingsSize);
			offset += c.settingsSize;
			free(c.settings);
			compressions.pop();
		}
		if ((offset > contentAddTotalSize) || (compressions.size() != 0))
			ALOGE("Parse content encoding error");
		ALOGD("contentAddData=0x%x, contentAddTotalSize=%d", *contentAddData, contentAddTotalSize);
	}
	
}

void Track::GetContentAddInfo(unsigned char** data, size_t* size)
{
	if (m_info.contentAddTotalSize != 0) {
		*size = m_info.contentAddTotalSize;
		*data = m_info.contentAddData;
		ALOGD("contentAddTotalSize=%d, contentAddData=0x%x(%p)", m_info.contentAddTotalSize, *(m_info.contentAddData), m_info.contentAddData);
	}
	else {
		*size = 0;
		*data = NULL;
...
#ifndef ANDROID_DEFAULT_CODE
		if (!strcmp("A_VORBIS",i.codecId) && i.codecPrivateSize != 0) {
			ALOGV("Vorbis audio has codec private EBML");
			if (m_rate == 0.0/*-1*/) {
				ALOGV("Error handle for no audio sample rate EBML header, get header from codec private data");
				long rate = 0;
				rate = *((long *)(i.codecPrivate + 15)); // (4 bytes header + sizeof("vorbis")+ 4 bytes version + one byte channels)
				m_rate = (double) rate;
				ALOGV("rate from codec private -> %d",(int)rate);
			}
			if (m_channels == 0/*-1*/) {
				ALOGV("Error handle for no audio channels EBML header, get header from codec private data");
				char channels;
				channels = *(i.codecPrivate + 14); // (4 bytes header + sizeof("vorbis")+ 4 bytes version)
				m_channels = (long long) channels;
				ALOGV("channels from codec private -> %d", (int)channels);
			}
		}
		ALOGV("Audio Track : sample rate -> %f, channels -> %lld, bits -> %lld",m_rate,m_channels,m_bitDepth);
#endif
    ...
    while (pos < stop)
    {
        ...
        else
        {
            ...
            else if (id == 0x33C5)  //Track UID
            {
				...
				ALOGD("Parse content encoding");
				i.ParseContentEncodings(pReader, pos - size, size);
			}
#endif
        }
    }
