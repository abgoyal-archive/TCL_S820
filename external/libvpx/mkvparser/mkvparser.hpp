...
#ifndef ANDROID_DEFAULT_DOCE
bool CheckUIntInvalid(long long data, long len);
#endif
struct EBMLHeader
{
    EBMLHeader();
...
public:
    ...
    long long GetTime(const Cluster*) const;      //absolute, and scaled (ns units)
    bool IsKey() const;
    void SetKey(bool);
...
public:
...
#ifndef ANDROID_DEFAULT_CODE
	void GetContentAddInfo(unsigned char** data, size_t* size);
#endif
    ...
    struct Info
    {
		...
		unsigned char* contentAddData;
		size_t contentAddTotalSize;

		void ParseContentEncodings(IMkvReader* pReader, long long pos, long long size);
#endif
        Info();
        void Clear();
    };
