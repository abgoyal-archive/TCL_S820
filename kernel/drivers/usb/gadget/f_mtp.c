...
//work around for DMA mode 0 + multi-package
...
//Added Modification for ALPS00354386
	...
	while (count > 0 || write_req) {
		...
		wait_request:
		        ...
		        ret = wait_event_interruptible_timeout(dev->read_wq, dev->rx_done || dev->state != STATE_BUSY, 5*HZ);
                        //work around for DMA mode 0 + multi-package
			if ( 0 == ret) {
			        struct musb_ep *musb_ep = to_musb_ep(dev->ep_out);
				...
				xlog_printk(ANDROID_LOG_WARN, "USB", "MTP read request timeout!\n");
				r = -ETIMEDOUT;
				
				spin_lock_irqsave(&musb->lock, flags);

				if (!musb_ep->dma){
				        xlog_printk(ANDROID_LOG_ERROR, "USB", "Endpoint not binded with any dma channel!\n");
					spin_unlock_irqrestore(&musb->lock, flags);
					usb_ep_dequeue(dev->ep_out, read_req);
					break;
				}				
				musb_channel = musb_ep->dma->private_data;
				bchannel = musb_channel->idx;

				if (MUSB_DMA_STATUS_BUSY != dma_channel_status(musb_ep->dma)){
				        //DMA channel is not busy but our request lost? rarely occur
					spin_unlock_irqrestore(&musb->lock, flags);
				        usb_ep_dequeue(dev->ep_out, read_req);
					break;
				}
				...
				if (!read_req->dma || 
				    read_req->dma != musb_channel->start_addr )
				{
				        //Our request has not been processed yet
					spin_unlock_irqrestore(&musb->lock, flags);
					r = 0;
					goto wait_request;
				}
                                ...
                                //Real copied bytes                     
				//FIXME:How to detect no pakeage or zero length package when actual length is 0?
				read_req->actual += musb_readl(musb->mregs, USB_DMA_REALCOUNT(bchannel));

				spin_unlock_irqrestore(&musb->lock, flags);
				xlog_printk(ANDROID_LOG_INFO, "USB", "MTP has received %d bytes!\n", read_req->actual);
				usb_ep_dequeue(dev->ep_out, read_req);
