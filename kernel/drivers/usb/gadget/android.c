...
static void android_disable(struct android_dev *dev)
{
	...
	xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: android_disable --> ", __func__);
	if (dev->disable_depth++ == 0) {
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: android_disable -> usb_gadget_disconnect", __func__);
		usb_gadget_disconnect(cdev->gadget);
		/* Cancel pending control requests */
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: android_disable -> usb_ep_dequeue", __func__);
		usb_ep_dequeue(cdev->gadget->ep0, cdev->req);
		xlog_printk(ANDROID_LOG_INFO, USB_LOG, "%s: android_disable -> usb_remove_config", __func__);
